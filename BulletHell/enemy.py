...
#================================================================================================================================
    ...
    def update(self, color=DEFAULT_ENEMY_COLOR):
        ...
        #    self.shoot_normal()
#======================================================================================
    PHASE_HEALTH = [5,30,30,30,30,30,30,30,30,5]
...
#=========================================================================================================
    ...
    def phase_shot(self, phase):
        ...
        if phase == 1:
            ...
            self.shoot_interval = 3000
            if self.current_time - self.last_shot_time >= self.shoot_interval:
                self.shoot_spread(4, 3, 40)
                self.delayer = 1
                self.last_shot_time = self.current_time
            if self.current_time - self.last_shot_time >= self.shoot_interval - 2500 and self.delayer == 1:
                self.shoot_spread(6, 5, 40)
                self.delayer = 2
            if self.current_time - self.last_shot_time >= self.shoot_interval - 2000 and self.delayer == 2:
                self.shoot_spread(8, 9, 50)
                self.delayer = 0
        
        elif phase == 6:
            self.shoot_interval = 3000
            if self.current_time - self.last_shot_time >= self.shoot_interval:
                self.shoot_line(8, 1, 10, 40)
                self.delayer = 1
                self.last_shot_time = self.current_time
            if self.current_time - self.last_shot_time >= self.shoot_interval - 2500 and self.delayer == 1:
                self.shoot_line(8, -1, 10, 40)
                self.delayer = 2
            if self.current_time - self.last_shot_time >= self.shoot_interval - 2000 and self.delayer == 2:
                self.shoot_line(7, 0, 10, 30, 0)
                self.delayer = 0
        elif phase == 7:
            self.shoot_interval = 3500
            if self.current_time - self.last_shot_time >= self.shoot_interval:
                self.shoot_line(6, -2, 10, 50)
                self.delayer = 1
                self.last_shot_time = self.current_time
            if self.current_time - self.last_shot_time >= self.shoot_interval - 3200 and self.delayer == 1:
                self.shoot_spread(6, 5, 40)
                self.delayer = 2
            if self.current_time - self.last_shot_time >= self.shoot_interval - 2800 and self.delayer == 2:
                self.shoot_line(6, 2, 10, 50)
                self.delayer = 3
            if self.current_time - self.last_shot_time >= self.shoot_interval - 2400 and self.delayer == 3:
                self.shoot_spread(8, 9, 40)
                self.delayer = 4
            if self.current_time - self.last_shot_time >= self.shoot_interval - 1600 and self.delayer == 4:
                self.shoot_line(7, 0, 10, 40, 0)
                self.delayer = 0
        elif phase == 8:
            self.shoot_interval = 600 
            if self.current_time - self.last_shot_time >= self.shoot_interval:
                self.shoot_spread(8, 8, 80)
                self.last_shot_time = self.current_time
    ...
    def shoot_spread(self, shots=PHASE_TWO_SHOTS, speed=PHASE_TWO_SPEED, size=PHASE_TWO_SIZE, rel=True, x=0, y=0):
        ...
        for i in range(shots):
            ...
            self.shoot_normal(rel, xPos, yPos, xSpeed, ySpeed, 0,0,size,size,0,0)
